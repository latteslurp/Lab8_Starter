describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    // first product item
    const product = await prodItems[0];
    let shadowRootProp = await product.getProperty('shadowRoot');
    let buttonElem = await shadowRootProp.$('button');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for(const product of prodItems){
      const shadowRootProp = await product.getProperty('shadowRoot');
      const buttonElem = await shadowRootProp.$('button');
      const innerText = await buttonElem.getProperty('innerText');
    ...
    const totalInCart = await page.evaluate(el => el.innerText, cart);
    expect(parseInt(totalInCart)).toBe(20);
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cart = await page.$('#cart-count');
    const totalInCart = await page.evaluate(el => el.innerText, cart);
    expect(parseInt(totalInCart)).toBe(0);
  }, 10000);
